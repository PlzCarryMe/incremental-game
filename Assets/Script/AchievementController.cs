...
public class AchievementController : MonoBehaviour
{
    ...
    public void UnlockAchievement2(AchievementType type, string value, double gold)
    {
        // Mencari data achievement
        AchievementData achievement2 = _achievementList.Find(a => a.Type == type && a.Value == value && a.gold <= gold);

        if (achievement2 != null && !achievement2.IsUnlocked)
        {
            achievement2.IsUnlocked = true;
            ShowAchivementPopUp(achievement2);
